...
- **Проверка синхронизации**: Обнаружение расхождений между профилем и конфигурацией git в реальном времени с предупреждением в статусной строке
...
| `gitIdSwitcher.syncCheck.enabled`          | `true`       | Проверять, совпадает ли выбранный профиль с текущей конфигурацией git                                             |
| `gitIdSwitcher.syncCheck.onFocusReturn`    | `true`       | Запускать проверку синхронизации при возврате фокуса в окно редактора                                             |
...
5. **Проверка синхронизации**: Проверяет, что применённый профиль совпадает с текущей конфигурацией git

### Проверка синхронизации

Сравнивает выбранный профиль с фактическими значениями `git config --local` (`user.name`, `user.email`, `user.signingkey`) и показывает предупреждение в статусной строке при обнаружении расхождения.

**Когда выполняются проверки:**

- Сразу после применения профиля
- При смене папки рабочей области
- При изменении конфигурации
- При возврате фокуса в окно редактора (с debounce 500 мс)

**При обнаружении расхождения:**

- В статусной строке отображается значок ⚠️ с предупреждающим фоном
- Подсказка показывает таблицу с расхождениями (поле, ожидаемое значение, фактическое значение)
- При нажатии на статусную строку предлагаются варианты решения:
  - **Повторно применить профиль** — Повторно применить текущий профиль к конфигурации git
  - **Выбрать другой профиль** — Открыть выбор профиля
  - **Отклонить** — Скрыть предупреждение до следующей проверки

**Для отключения:**

Установите `gitIdSwitcher.syncCheck.enabled` в `false`, чтобы отключить все проверки синхронизации.
Чтобы отключить только проверку при возврате фокуса, установите `gitIdSwitcher.syncCheck.onFocusReturn` в `false`.
