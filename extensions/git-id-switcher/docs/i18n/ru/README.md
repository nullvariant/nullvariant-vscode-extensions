...
1. **Кошмар подмодулей**: При работе с репозиториями, содержащими подмодули (темы Hugo, vendor-библиотеки и т.д.), обычно приходится вручную настраивать `git config user.name` для _каждого_ подмодуля. Это расширение элегантно решает проблему, рекурсивно применяя ваш идентификатор ко всем активным подмодулям.
...
| `gitIdSwitcher.logging.redactAllSensitive` | `false`      | Когда включено, все значения маскируются в журналах (максимальная конфиденциальность)              |
...
| Команда                                  | Описание                         |
| ---------------------------------------- | -------------------------------- |
| `Git ID Switcher: Select Identity`       | Открыть селектор идентификаторов |
| `Git ID Switcher: Show Current Identity` | Показать текущий идентификатор   |
