...
1. **Кошмар підмодулів**: При роботі з репозиторіями, що містять підмодулі (теми Hugo, vendor-бібліотеки тощо), зазвичай потрібно вручну налаштовувати `git config user.name` для _кожного_ підмодуля. Це розширення елегантно вирішує проблему, рекурсивно застосовуючи ваш ідентифікатор до всіх активних підмодулів.
...
| `gitIdSwitcher.logging.redactAllSensitive` | `false`          | Коли ввімкнено, всі значення маскуються у журналах (максимальна конфіденційність)                  |
...
| Команда                                  | Опис                              |
| ---------------------------------------- | --------------------------------- |
| `Git ID Switcher: Select Identity`       | Відкрити селектор ідентифікаторів |
| `Git ID Switcher: Show Current Identity` | Показати поточний ідентифікатор   |
