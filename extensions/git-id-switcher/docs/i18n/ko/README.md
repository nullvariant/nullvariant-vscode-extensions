...
Git ID Switcher는 VS Code의 [워크스페이스 신뢰](https://code.visualstudio.com/docs/editor/workspace-trust) 기능을 지원합니다. 신뢰할 수 없는 워크스페이스에서는 위험한 작업이 자동으로 제한됩니다.

### 신뢰할 수 없는 워크스페이스에서의 동작
...
부득이하게 워크스페이스 신뢰 검사를 비활성화해야 하는 경우:

1. 설정 열기 (`Cmd+,` / `Ctrl+,`)
2. `gitIdSwitcher.disableWorkspaceTrust` 검색
3. 설정 활성화

⚠️ **경고**: 이 설정을 활성화하면 보안이 저하됩니다. 반드시 필요한 경우에만 사용하세요.
