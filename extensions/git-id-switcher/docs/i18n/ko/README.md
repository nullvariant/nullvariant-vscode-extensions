...
> JSON 설정 예시는 「[전체 설정 예시](#전체-설정-예시-4개-계정과-ssh--gpg)」를 참조하세요.

---

## 전체 설정 예시: 4개 계정과 SSH + GPG
...
참고: 마지막 프로필(`freelance`)에는 SSH가 없습니다. 동일한 GitHub 계정에서 다른 커미터 정보를 사용하는 경우 등, Git config만 전환하는 것도 가능합니다.
