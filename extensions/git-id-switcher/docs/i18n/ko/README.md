...
- **동기화 체크**: 프로필과 git config 간의 불일치를 실시간으로 감지하여 상태 표시줄에 경고 표시
...
| `gitIdSwitcher.syncCheck.enabled`          | `true`     | 선택한 프로필이 실제 git config와 일치하는지 확인                                   |
| `gitIdSwitcher.syncCheck.onFocusReturn`    | `true`     | 에디터 창이 포커스를 다시 받을 때 동기화 체크 실행                                  |
...
5. **동기화 체크**: 적용된 프로필이 실제 git config와 일치하는지 확인

### 동기화 체크

선택한 프로필을 실제 `git config --local` 값 (`user.name`, `user.email`, `user.signingkey`)과 비교하고, 불일치가 감지되면 상태 표시줄에 경고를 표시합니다.
...
- 워크스페이스 폴더 변경 시
- 설정 변경 시
- 에디터 창 포커스 복귀 시 (500ms 디바운스)

**불일치가 감지된 경우:**

- 상태 표시줄에 ⚠️ 아이콘과 경고 배경색이 표시됩니다
- 툴팁에 불일치 필드(필드명, 기대값, 실제값)를 보여주는 테이블이 표시됩니다
- 상태 표시줄을 클릭하면 해결 옵션이 제시됩니다:
  - **프로필 재적용** — 현재 프로필을 git config에 다시 적용
  - **다른 프로필 선택** — 프로필 선택기 열기
  - **무시** — 다음 체크까지 경고 숨기기

**비활성화 방법:**

`gitIdSwitcher.syncCheck.enabled`를 `false`로 설정하면 모든 동기화 체크가 비활성화됩니다.
포커스 복귀 시의 체크만 비활성화하려면 `gitIdSwitcher.syncCheck.onFocusReturn`을 `false`로 설정하세요.
