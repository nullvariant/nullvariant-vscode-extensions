...
Git ID Switcher 支援 VS Code 的[工作區信任](https://code.visualstudio.com/docs/editor/workspace-trust)功能。在不受信任的工作區中，危險操作會被自動限制。
...
1. 開啟設定（`Cmd+,` / `Ctrl+,`）
2. 搜尋 `gitIdSwitcher.disableWorkspaceTrust`
3. 啟用該設定

⚠️ **警告：** 啟用此設定會降低安全性。請僅在絕對必要時使用。
