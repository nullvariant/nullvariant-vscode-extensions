...
- **SYNC CHEK**: REEL-TIEM DETECSHUN OF MISMATCHZ BETWEEN PROFILE AN GIT CONFIG, WIF STATUS BAR WARNIN. O NOEZ!
...
| `gitIdSwitcher.syncCheck.enabled`          | `true`     | CHEK WHETHR SELECTD PROFILE MATCHZ ACTUAL GIT CONFIG                                        |
| `gitIdSwitcher.syncCheck.onFocusReturn`    | `true`     | RUN SYNC CHEK WEN EDITR WINDO REGAINZ FOCUS                                                 |
...
5. **SYNC CHEK**: VERIFYZ DAT TEH APPLYD PROFILE MATCHZ TEH ACTUAL GIT CONFIG

### SYNC CHEK

COMPAREZ UR SELECTD PROFILE AGAINST TEH ACTUAL `git config --local` VALUEZ (`user.name`, `user.email`, `user.signingkey`) AN SHOWZ A STATUS BAR WARNIN WEN A MISMATCH IZ DETECTD. NOT COOL!

**WEN CHEKZ RUN:**

- RITE AFTR APPLYIN PROFILE
- ON WERKSPACE FOLDR CHANGE
- ON CONFIGURASHUN CHANGE
- ON EDITR WINDO FOCUS RETURN (DEBOUNSD 500MS)

**WEN A MISMATCH IZ DETECTD:**

- TEH STATUS BAR SHOWZ A ⚠️ ICON WIF A WARNIN BAKGROUND COLR. SCAREE!
- TEH TOOLTIP DISPLAYZ A TABL SHOWIN TEH MISMATCHD FIELDZ (FIELD, EXPECTD VALU, ACTUAL VALU)
- CLIKIN TEH STATUS BAR PRESENTZ RESOLUSHUN OPSHUNZ:
  - **RE-APPLY PROFILE** — RE-APPLY TEH CURRENT PROFILE 2 GIT CONFIG
  - **SELECT DIFFRNT PROFILE** — OPEN TEH IDENTITEE PIKR
  - **DISMISS** — SUPPRESS TEH WARNIN UNTIL TEH NEXT CHEK

**2 DISABL:**

SET `gitIdSwitcher.syncCheck.enabled` 2 `false` 2 DISABL ALL SYNC CHEKZ.
2 DISABL ONLY TEH FOCUS-RETURN CHEK, SET `gitIdSwitcher.syncCheck.onFocusReturn` 2 `false`.
