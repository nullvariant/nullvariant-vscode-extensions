...
- **Sync Check**: プロフィールとぅgit configぬ不一致ばリアルタイムんかい検出さーに、ステータスバーんかい警告表示
...
| `gitIdSwitcher.syncCheck.enabled`          | `true`       | 選択したプロフィールが実際ぬgit configとぅ一致しちょーるか確認                                            |
| `gitIdSwitcher.syncCheck.onFocusReturn`    | `true`       | エディタウィンドウがフォーカス戻った時んかいsync check実行                                                |
...
5. **Sync Check**: 適用したプロフィールが実際ぬgit configとぅ一致しちょーるか確認

### Sync Check

選択したプロフィールば実際ぬ`git config --local`ぬ値（`user.name`, `user.email`, `user.signingkey`）とぅ比較さーに、不一致が検出さりたらステータスバーんかい警告表示さびーん。
...
- ワークスペースフォルダ変更時
- 設定変更時
- エディタウィンドウがフォーカス戻った時（500ミリ秒デバウンス）

**不一致が検出さりた場合:**

- ステータスバーんかい⚠️アイコンとぅ警告背景色が表示さーるん
- ツールチップんかい不一致フィールドぬテーブル表示（フィールド、期待値、実際値）
- ステータスバークリックさーに解決オプション表示:
  - **プロフィール再適用** — 今ぬプロフィールばgit configんかい再適用
  - **別ぬプロフィール選択** — プロフィールピッカー開ちゅん
  - **無視** — 次ぬ確認まで警告ば非表示

**無効にすーる方法:**

`gitIdSwitcher.syncCheck.enabled`ば`false`んかい設定すーると、全部ぬsync checkが無効なーいびーん。
フォーカス戻り時ぬ確認だけ無効にしーぶさい場合や、`gitIdSwitcher.syncCheck.onFocusReturn`ば`false`んかい設定しくぃみそーれー。
