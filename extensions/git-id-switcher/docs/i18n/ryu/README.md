...
Git ID Switcher や VS Code ぬ [ワークスペースの信頼](https://code.visualstudio.com/docs/editor/workspace-trust) 機能んかい対応しちょーいびーん。信頼さってぃない ワークスペースやったら、危ない操作が自動的んかい制限さーいびーん。

### 信頼さってぃないワークスペースんかいぬ動作
...
どーしても ワークスペース信頼チェック 無効んかいしたかったら：

1. 設定開ちゅん（`Cmd+,` / `Ctrl+,`）
2. `gitIdSwitcher.disableWorkspaceTrust` 探すん
3. 設定有効んかいしゅん

⚠️ **注意:** くぬ設定有効んかいしたらセキュリティ下がいびーん。必要最小限んかいしくぃみそーれー。
