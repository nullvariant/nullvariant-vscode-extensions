...
Типична настройка за управление на множество GitHub акаунти.
...
Отворете настройките на VS Code (`Cmd+,` / `Ctrl+,`) → потърсете "Git ID Switcher" → кликнете на "Редактиране в settings.json":
...
```json
{
  "gitIdSwitcher.identities": [
    ...
    },
    {
      ...
      "description": "Open source приноси",
...
Забележка: Последната самоличност (`freelance`) е без SSH — превключва само Git конфигурацията. Това е полезно когато използвате различна committer информация със същия GitHub акаунт.
...
| `icon`        |              | Емоджи в лентата на състоянието (напр.: `"💼"`)             |
